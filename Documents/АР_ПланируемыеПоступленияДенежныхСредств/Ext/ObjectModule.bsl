...
Процедура ЗаполнитьПоДействующимДоговорам() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ДокСчет = Документы.АР17_СчетНаАренду.СоздатьДокумент();
		ДокСчет.Дата = ТекущаяДата();
		ЗаполнитьЗначенияСвойств(ДокСчет, Выборка);
		
		СтруктураКурса = РаботаСКурсамиВалют.ПолучитьКурсВалюты(ДокСчет.ВалютаДокумента, Дата);
...
Процедура ЗаполнитьПоДаннымПрошлыхПериодов(мНастройка) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.АР_ВзаиморасчетыПоАренде.Обороты(&НачалоПериода, &ОкончаниеПериода, , ) КАК АР_ВзаиморасчетыПоАрендеОбороты
	|
	|СГРУППИРОВАТЬ ПО
