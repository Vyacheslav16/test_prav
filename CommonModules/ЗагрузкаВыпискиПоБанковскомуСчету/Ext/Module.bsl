...
Функция ВыделитьНДСИзНазначенияПлатежа(ИсходноеНазначениеПлатежа, СуммаПлатежа) Экспорт
	...
	Иначе
		...
		ТокенСумма = НСтр("ru = 'сумма'");
		Если СтрНачинаетсяС(СокрЛ(ИнформацияОСумме), ТокенСумма) Тогда
			ИнформацияОСумме = СокрЛП(СтрЗаменить(ИнформацияОСумме, ТокенСумма, ""));
		КонецЕсли;
...
Функция ХозяйственнаяОперацияВыручкаРозничнойТорговлиЧерезПосредника(Параметры)
	...
	Если ЗначениеЗаполнено(СуммаУслугБанка) Тогда
		ИнформацияОНДС = ВыделитьНДСИзНазначенияПлатежа(Параметры.Операция.НазначениеПлатежа, СуммаУслугБанка);
...
Функция ПоискШаблонаВТекстеНазначения(Текст, Шаблоны)
	...
	Для Каждого Шаблон Из Шаблоны Цикл
		Если ЭтоШаблоныНДС Тогда
			...
			Если СтрНачинаетсяС(СокрЛ(РезультатПоиска.ТекстПосле), ТокенУплаты) Тогда
				РезультатПоиска.ТекстПосле = СокрЛП(СтрЗаменить(РезультатПоиска.ТекстПосле, ТокенУплаты, ""));
			КонецЕсли;
