...
Процедура СлужебнаяУстановитьПрефиксИБИПерепрефиксоватьОбъектыСозданныеВЭтойИБ(
	...
	Если Не СтандартныеПодсистемыПовтИсп.РазделениеВключено() Тогда
	...
	Для Каждого Подписка Из ПодпискиНаСобытияПоИмениОбработчика(ИмяОбработчика) Цикл
		...
		Для Каждого ТипИсточника Из Подписка.Источник.Типы() Цикл
			...
			ИначеЕсли РазделениеВключено Тогда
				...
				Если Не РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(ИмяОбъекта, РаботаВМоделиСервиса.РазделительВспомогательныхДанных())
					И Не РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(ИмяОбъекта, РаботаВМоделиСервиса.РазделительОсновныхДанных())Тогда
