...
// Заполняет во входящих и исходящих письмах реквизиты Контрагент, КонтактноеЛицо и Ответственный
//
// Параметры:
//  МассивПисем - Массив из ДокументСсылка.ЭлектронноеПисьмоВходящее, ДокументСсылка.ЭлектронноеПисьмоИсходящее -
//                содержит ссылки на письма, которые необходимо заполнить
//
Процедура ДозаполнитьПисьма(МассивПисем) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ИСТИНА КАК ЭтоВходящееПисьмо,
		|	ЭлектронноеПисьмоВходящее.Ссылка КАК ЭлектронноеПисьмо,
		|	ЭлектронноеПисьмоВходящее.ОтправительАдрес КАК АдресОтправителяПолучателя,
		...
		|	ЕСТЬNULL(УчетныеЗаписиЭлектроннойПочты.ВладелецУчетнойЗаписи, ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка))
		|ИЗ
		...
		|	Контакты.ЭтоВходящееПисьмо КАК ЭтоВходящееПисьмо,
		|	Контакты.ЭлектронноеПисьмо КАК ЭлектронноеПисьмо,
		...
		|			ПО (НРЕГ(ВЫРАЗИТЬ(КонтрагентыКонтактнаяИнформация.Представление КАК СТРОКА(100))) = НРЕГ(Контакты.АдресОтправителяПолучателя))
		...
		|		Контакты.ЭтоВходящееПисьмо,
		|		Контакты.ЭлектронноеПисьмо,
		...
		|			ПО (НРЕГ(ВЫРАЗИТЬ(КонтактныеЛицаКонтактнаяИнформация.Представление КАК СТРОКА(100))) = НРЕГ(Контакты.АдресОтправителяПолучателя))
	...
	ВыборкаЭлектронноеПисьмо = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	...
	Пока ВыборкаЭлектронноеПисьмо.Следующий() Цикл
		ЭлектронноеПисьмоОбъект = ВыборкаЭлектронноеПисьмо.ЭлектронноеПисьмо.ПолучитьОбъект();
		КонтактнаяИнформацияВыборка = ВыборкаЭлектронноеПисьмо.Выбрать();
		Если КонтактнаяИнформацияВыборка.Следующий() Тогда
			Если ЗначениеЗаполнено(КонтактнаяИнформацияВыборка.ВзаимодействиеОснование) Тогда
				Если КонтактнаяИнформацияВыборка.ЭтоВходящееПисьмо Тогда
					ЭлектронноеПисьмоОбъект.Предмет = КонтактнаяИнформацияВыборка.ВзаимодействиеОснование;
				Иначе
					Если ЭлектронноеПисьмоОбъект.ДокументыОснования.Найти(КонтактнаяИнформацияВыборка.ВзаимодействиеОснование) <> Неопределено Тогда
						НоваяСтрокаОснования = ЭлектронноеПисьмоОбъект.ДокументыОснования.Добавить();
						НоваяСтрокаОснования.ДокументОснование = КонтактнаяИнформацияВыборка.ВзаимодействиеОснование;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			ЗаполнитьЗначенияСвойств(ЭлектронноеПисьмоОбъект, КонтактнаяИнформацияВыборка, "Контрагент, КонтактноеЛицо");
			Если ЗначениеЗаполнено(КонтактнаяИнформацияВыборка.Ответственный) Тогда
				ЭлектронноеПисьмоОбъект.Ответственный = КонтактнаяИнформацияВыборка.Ответственный;
			ИначеЕсли Не ЗначениеЗаполнено(ЭлектронноеПисьмоОбъект.Ответственный) Тогда
		...
		Исключение
			ТекстОшибки = СтрШаблон(НСтр("ru = 'При изменении клиента в электронном письме произошла ошибка:'
