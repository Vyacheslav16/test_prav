...
Процедура ЗаполнитьОсновнуюЧастьУКД_2020(ДанныеДляФормированияЭД, Настройки, ДеревоДанных, Отказ = Ложь)
	...
	Если СведенияОПоставщике.ЭтоИндивидуальныйПредприниматель
			И Не ЗначениеЗаполнено(СведенияОПоставщике.ДатаПрисвоенияОГРН) Тогда
		ТекстОшибки = НСтр("ru = 'Не указана дата присвоения ОГРНИП в карточке организации'");
			СообщитьОбОшибкеПриФормированииЭД(ДанныеДокументаИСчетаФактуры.Документ, ТекстОшибки, Отказ);
	КонецЕсли;
...
Функция ДанныеУчастникаЭД(ПараметрыУчастника, Отказ = Ложь)
	...
	Иначе
		...
		Если ТипУчастника = "ИП" Тогда
			...
			Если Не ЗначениеЗаполнено(СведенияОбУчастнике.ДатаПрисвоенияОГРН)
					И ПараметрыУчастника.ВидУчастника = ВидыУчастниковЭД().Продавец Тогда
				ТекстОшибки = НСтр("ru = 'Не указана дата присвоения ОГРНИП в карточке организации'");
				СообщитьОбОшибкеПриФормированииЭД(ПараметрыУчастника.ДокументУчета, ТекстОшибки, Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
