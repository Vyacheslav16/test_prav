...
Функция сбисПолучитьФорму(ИмяФормы)
	Если ТипЗнч(ЭтаФорма) = Тип("УправляемаяФорма") Тогда
		...
		Исключение
		КонецПопытки;
		Возврат ПолучитьФорму("Обработка.Д_ВыгрузкаНаПортал.Форма."+ИмяФормы);
	КонецЕсли;
...
Процедура ОтправитьДокументы(Кэш,МассивСтрок) Экспорт
	...
	Если ТипЗнч(РезультатОтправки) = Тип("Структура") Тогда  // если необходимо активировать сертификаты перед отправкой
		...
		Для Каждого Элемент Из РезультатОтправки.СписокСертификатов Цикл
			...
			ОткрытьФорму("Обработка.Д_ВыгрузкаНаПортал.Форма.ФормаВводаПинкода",Новый Структура("СертификатИмя,счСертификатов",СертификатИмя,счСертификатов),ЭтаФорма,счСертификатов,,,Новый ОписаниеОповещения("АктивироватьСертификатыЗавершение",ЭтаФорма,Новый Структура("Кэш,РезультатОтправки",Кэш,РезультатОтправки)));	
...
Функция ОтправитьДокументыПоМассивуСтрок(Кэш,МассивСтрок,счСтрок,МассивПакетов,ИННОтправителей, УжеАктивированы) Экспорт
	...
	Пока счСтрок<Всего Цикл //Отправляемые пакеты
		...
		МассивПодготовленныхПакетов = Кэш.Текущий.Форма.ПодготовитьСтруктуруДокумента(Строка.Значение, Кэш);
...
Функция ОтправитьПодготовленныеПакетыДокументы(Кэш, МассивПодготовленныхПакетов) Экспорт
	...
	Если ТипЗнч(РезультатОтправки) = Тип("Структура") Тогда  // если необходимо активировать сертификаты перед отправкой
		...
		Для Каждого Элемент Из РезультатОтправки.СписокСертификатов Цикл
			...
			КонецЕсли;
			СертификатИмя = "Введите код активации сертификата "+Кэш.ОбщиеФункции.СокращенноеФИО(Кэш,Сертификат.ФИО)+?(Сертификат.Свойство("Должность"),"("+Сертификат.Должность+")", "")+", "+Сертификат.Название+?(Сертификат.Свойство("ИНН"),"(ИНН "+Сертификат.ИНН+")","");
			ОткрытьФорму("Обработка.Д_ВыгрузкаНаПортал.Форма.ФормаВводаПинкода",Новый Структура("СертификатИмя,счСертификатов",СертификатИмя,счСертификатов),ЭтаФорма,,,,Новый ОписаниеОповещения("АктивироватьСертификатыЗавершение",ЭтаФорма,Новый Структура("Кэш,РезультатОтправки",Кэш,РезультатОтправки)));	
			счСертификатов = счСертификатов+1;
		КонецЦикла;
	КонецЕсли;	
КонецФункции
...
Функция ОтправитьПодготовленныеДокументы(Кэш, МассивПодготовленныхПакетов,счПодготовленныхПакетов,МассивСтрок,счСтрок, МассивПакетов, ИННОтправителей, УжеАктивированы, ПоследняяПартия) Экспорт
	...
	Пока счПодготовленныхПакетов<КолПакетов Цикл
		...
		Иначе
			Если ТипЗнч(ПолныйСоставПакета.НашаОрганизация) = Тип("СправочникСсылка.Организации") Тогда
				ИННОтправителя = ПолучитьИНН(ПолныйСоставПакета.НашаОрганизация);
			Иначе
				ИННОтправителя = ПолучитьИННСвЮЛ(ПолныйСоставПакета.НашаОрганизация);//?(ПолныйСоставПакета.НашаОрганизация.Свойство("СвФЛ"),ПолныйСоставПакета.НашаОрганизация.СвФЛ.ИНН,ПолныйСоставПакета.НашаОрганизация.СвЮЛ.ИНН);
			КонецЕсли;
...
Функция ПолучитьИНН(НашаОрганизация)
	Возврат НашаОрганизация.ИНН;	
КонецФункции

Функция ПолучитьИННСвЮЛ(НашаОрганизация)
	Возврат НашаОрганизация.СвЮЛ.ИНН;	
КонецФункции
		...
		Если СчетчикВведенныхПинкодов = РезультатОтправки.СписокСертификатов.Количество() Тогда   // если ввели все пин-коды
			...
			Если ТипЗнч(ПромежуточныйРезультатОтправки) = Тип("Структура") Тогда
				...
				Для Каждого Элемент Из ПромежуточныйРезультатОтправки.СписокСертификатов Цикл
					...
					КонецЕсли;
					СертификатИмя = "Введите код активации сертификата "+Кэш.ОбщиеФункции.СокращенноеФИО(Кэш,Сертификат.ФИО)+?(Сертификат.Свойство("Должность"),"("+Сертификат.Должность+")", "")+", "+Сертификат.Название+?(Сертификат.Свойство("ИНН"),"(ИНН "+Сертификат.ИНН+")","");
					ОткрытьФорму("Обработка.Д_ВыгрузкаНаПортал.Форма.ФормаВводаПинкода",Новый Структура("СертификатИмя,счСертификатов",СертификатИмя,счСертификатов),ЭтаФорма,,,,Новый ОписаниеОповещения("АктивироватьСертификатыЗавершение",ЭтаФорма,Новый Структура("Кэш,РезультатОтправки",Кэш,ПромежуточныйРезультатОтправки)));	
					счСертификатов = счСертификатов+1;
				КонецЦикла;
				Возврат Истина;
			КонецЕсли;
...
Процедура СбисВычислитьПолеНаСервере(Знач СбисПоле, Знач АргументыРассчитать, ДанныеЗаполнить)
	...
	ИначеЕсли	СбисПоле.Свойство("РассчитанноеЗначение", РезультатРасчета) Тогда
		Если	АргументыРассчитать.Свойство("ЗаполнитьРасчитанноеЗначение")
			И	АргументыРассчитать.ЗаполнитьРасчитанноеЗначение
			И	ЗначениеЗаполнено(РезультатРасчета) Тогда
...
//Контекст{ини,Документ,СтрТабл}
&НаКлиенте
Функция РассчитатьЗначениеПараметра(Параметр,Контекст,Кэш) Экспорт 
...
Функция СоздатьНаборСвойствПоМассиву(РассчитанныйНаборСвойств, МассивКлючей, ИниКонфигурация, Отказ) Экспорт
	...
	Для Каждого Ключ ИЗ МассивКлючей Цикл
		...
		ДопСведение = НайтиДопСведение(СтруктураОбъекта, ИмяПланаВидовХарактеристикСвойствОбъектов, НаборСвойств, ОбщееДляНесколькихНаборов, Отказ);
...
&НаСервереБезКонтекста
Функция НайтиДопСведение(СтруктураОбъекта, ИмяПланаВидовХарактеристикСвойствОбъектов, НаборСвойств, ОбщееДляНесколькихНаборов, Отказ)
...
Процедура ОбработатьЭлементСУ_Сервер(Строка, СтруктураДанных, ТабЧастьСтруктура, ДокОбъект, ДопПараметры)
	...
	КонтекстРасчета = Новый Структура(
	"Ини,				СтрокаФайла,СтрТабл,			Документ, ЗаполнитьРасчитанноеЗначение, СтруктураФайла, Переменные, ТекущееПолеКлюч",
	КонтекстИниСтроки,	Строка,		КонтекстИниСтроки,	ДокОбъект,Истина);
