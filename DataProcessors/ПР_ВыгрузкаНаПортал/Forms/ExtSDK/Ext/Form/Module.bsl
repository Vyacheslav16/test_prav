...
Функция Отправка_ПодготовитьСтруктуруПакета(Кэш, ПараметрыПодготовки, ДопПараметры, Отказ) Экспорт
	...
	org = Новый Структура; 
	НОрг = ПолучитьОрг(СоставПакета.НашаОрганизация);
	Если НОрг.ЭтоОрг Тогда
		СвЮЛ = Новый Структура;
		СвЮЛ.Вставить( "ИНН", НОрг.ИНН ); 
		СвЮЛ.Вставить( "КПП", НОрг.КПП );
		org.Вставить( "СвЮЛ", СвЮЛ );
	Иначе	
	...
	НОрг = ПолучитьКонтр(СоставПакета.Контрагент);
	Если НОрг.ЭтоОрг Тогда
		kontr = Новый Структура; 
		СвЮЛ = Новый Структура;
		СвЮЛ.Вставить( "ИНН", НОрг.ИНН ); 
...
Функция ПолучитьОрг(Организация)                                        
	ЭтоОрг = Новый Структура;
	Если ТипЗнч(Организация) = Тип("СправочникСсылка.Организации") Тогда
		ЭтоОрг.Вставить("ЭтоОрг",Истина);
		ЭтоОрг.Вставить("ИНН",Организация.ИНН);
		ЭтоОрг.Вставить("КПП",Организация.КПП); 
	Иначе 
		ЭтоОрг.Вставить("ЭтоОрг",Ложь);
	КонецЕсли;
    Возврат ЭтоОрг;
КонецФункции

Функция ПолучитьКонтр(Организация)                                        
	ЭтоОрг = Новый Структура;
