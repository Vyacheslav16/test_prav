...
Функция сбисПолучитьФорму(ИмяФормы, Объект1С = Неопределено, ПараметрыФормы = Неопределено, ВладелецФормы = Неопределено)
	Если ТипЗнч(ЭтаФорма) = Тип("УправляемаяФорма") Тогда
		...
		Исключение
		КонецПопытки;
		Возврат ПолучитьФорму("Обработка.Д_ВыгрузкаНаПортал.Форма."+ИмяФормы, ПараметрыФормы);
	КонецЕсли;
	...
	Если ФормаПросмотра.СоставПакета.Свойство("Вложение") Тогда
		...
		Для Каждого Документ Из Документы1С Цикл
			...
			Иначе
				...
				ОткрытьФорму("Обработка.Д_ВыгрузкаНаПортал.Форма.Форма_ВыборИзСписка",Новый Структура("СписокДляВыбора, Заголовок",СписокИни, "Выберите настройку для формирования документов для "+Документ.Значение),,,,,Оповещение);
			КонецЕсли;
		КонецЦикла;
		...
		Если РезультатЗапроса.Колонки.Найти("Приложение")<>Неопределено Тогда
			Пока Выборка.Следующий() Цикл //перебираем все связанные документы
				Если ЗначениеЗаполнено(Выборка.Приложение) Тогда //если есть связанный документ
					...
					Если ТипЗнч(Выборка.Приложение) <> Тип("ДокументСсылка.АР17_НачислениеАренднойПлаты")
						Тогда СписокДокументов.Добавить(Выборка.Приложение,ФайлНастроек);
					КонецЕсли;
					// Клес -
				КонецЕсли;	
			КонецЦикла;
...
#Область include_core_vo2_ОбщиеФункции_ЗагрузкаДокументов_Функции
...
Функция ЭтоОтчет(Док)
	Возврат ТипЗнч(Док) = Тип("ДокументСсылка.АР17_Д_ОтчетИнвестора");	
КонецФункции	

Функция ПолучитьТипДок(Док)
	Если ТипЗнч(Док) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		//Если Док.Товары.Количество() > 0 Тогда
		Возврат "УПД";
		//Возврат "УпдСчфДоп";
	Иначе
		Возврат Сред(Метаданные.НайтиПоТипу(ТипЗнч(Док)).ПолноеИмя(),10);
	КонецЕсли;
	//КонецЕсли;	
	Возврат Сред(Метаданные.НайтиПоТипу(ТипЗнч(Док)).ПолноеИмя(),10);
КонецФункции

Функция ЭтоУПД(Документ)    
	Если ТипЗнч(Документ) = Тип("Массив") Тогда
		Для каждого стр Из Документ Цикл
			Док = стр.Значение;	
			Если Не ТипЗнч(Док) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;
		Возврат Истина; 
...
Функция ПодготовитьСтруктуруДокумента1С(СтрокаСпискаДокументов, Кэш) Экспорт
	...
	ИниТекущегоРеестра = ПолучитьТипДок(СтрокаСпискаДокументов[0].Документ);	
	//ИниТекущегоРеестра = Кэш.Текущий.ТипДок;
	Если ЭтоОтчет(СтрокаСпискаДокументов[0].Документ) Тогда
		ТипПакета = "КоррИсх";
	Иначе	
	...
	Для каждого Элемент из СтрокаСпискаДокументов[0].СоставПакета Цикл		
	...
	КонецЕсли;
	ПолучитьДанныеДокументов1С(СоответствиеДокументДанные, Кэш.КэшЗначенийИни, СтрокаСпискаДокументов[0].СоставПакета, Кэш.Парам);  // передаем СоставПакета, так как в нем нужный порядок документов	// alo Меркурий
	Если Кэш.Свойство("РезультатОтправки") Тогда
	...
	Для каждого Элемент из СтрокаСпискаДокументов[0].СоставПакета Цикл		
		...
		Если ТипЗнч(Документ) <> Тип("Строка") Тогда
			...
			КонецЕсли;  
		Иначе
			Контекст.Вставить("Документ", "Отчет");
			Контекст.Вставить("ДокументДанные", "Отчет");
			фрм = ГлавноеОкно.сбисНайтиФормуФункции("ПрочитатьДокумент","Документ_"+ИмяДокумента,"Документ_Шаблон", Кэш);
			Если Не фрм.ПрочитатьДокумент(Кэш,Контекст) Тогда
				Возврат Новый Массив;
			КонецЕсли;  
	...
	Для Каждого Пакет Из Контекст.МассивПакетов Цикл
			...
			ИначеЕсли Вложение.Свойство("СтруктураДокумента") Тогда // формируем xml документов
				...
				Если Кэш.Парам.ИспользоватьГенератор 
					...
					Если Не фрм = Ложь Тогда
						...
						РезультатПодставновки = фрм.ПрименитьПодстановку(Кэш, Вложение, ОшибкаФормирования);
				...
				// Клес +
				ДополнитьШаблонДляАшан(ШаблонXML, СтруктураВложения); 
				// Клес -
		...
		Если МассивВложений.Количество()>0 Тогда
			...
			Если НЕ Пакет.Свойство("Конверт") Тогда  // если нет структуры конверта, то берем все с первого вложения
				...
				КонецЕсли;
				ОсновнойДокумент = СтрокаСпискаДокументов[0].СоставПакета[0].Значение;
...
Функция ПолучитьДанныеДокументов1С(СоответствиеДокументДанные, КэшЗначенийИни, СоставПакета, КэшПарам=неопределено) Экспорт	// alo Меркурий
	...
	Для каждого Док из СоставПакета Цикл
		...
		//+Конюшок СН 092020 группировка агентских услуг
		С_ПроверкаГруппировкаАгентскихУслугПоАренде(ГрязныйИни, Документ);
		//-Конюшок СН 092020 группировка агентских услуг
...
#Область include_core_vo2_ОбщиеФункции_ЗагрузкаДокументов_ВыгрузкаДокументов
...
#Область С_ГруппировкаАгентскихУслуг
//+Конюшок СН 092020 группировка агентских услуг
&НаСервере
Процедура С_ПроверкаГруппировкаАгентскихУслугПоАренде(Ини, Документ)
	
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И ЗначениеЗаполнено(Документ.СчетНаОплатуПокупателю) 
		И ТипЗнч(Документ.СчетНаОплатуПокупателю) = Тип("ДокументСсылка.АР17_НачислениеАренднойПлаты") Тогда
		
		С_СвернутьТаблицыУслуг(Ини, Документ, 1);
		
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.КорректировкаРеализации")
		И ТипЗнч(Документ.ДокументРеализации) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		И ЗначениеЗаполнено(Документ.ДокументРеализации.СчетНаОплатуПокупателю) 
		И ТипЗнч(Документ.ДокументРеализации.СчетНаОплатуПокупателю) = Тип("ДокументСсылка.АР17_НачислениеАренднойПлаты") Тогда
		
		С_СвернутьТаблицыУслуг(Ини, Документ, 2);
...
&НаСервере
Процедура ЗаполнитьЕдИзмПоАктамАренды(МассивУслуг, Документ, ТипДокумента)
	
	ЕдИзмУсл = Справочники.КлассификаторЕдиницИзмерения.ПолучитьСсылку(Новый УникальныйИдентификатор("fa76fe5a-c6e4-11df-99d8-6c626d0f3f48"));
	
	Если ТипДокумента = 1 Тогда 
	
		Для Каждого Стр Из МассивУслуг Цикл
			
			МСтр = Документ.СчетНаОплатуПокупателю.Состав.НайтиСтроки(Новый Структура("Номенклатура, Содержание", Стр.Номенклатура, Стр.Описание));
			Если МСтр.Количество() > 0 Тогда
				Стр.ЕдИзм = МСтр[0].ЕдиницаИзмерения;
				Стр.ТаблДок_ОКЕИ = Стр.ЕдИзм.Код; 
				Стр.ТаблДок_ЕдИзм = Строка(Стр.ЕдИзм);
			КонецЕсли;
			
			Если Стр.ЕдИзм = ЕдИзмУсл Тогда
				Стр.ЕдИзм = Неопределено;
				Стр.ТаблДок_ОКЕИ = "";
				Стр.ТаблДок_ЕдИзм = "";
			КонецЕсли;
	...
	ИначеЕсли ТипДокумента = 2 Тогда
		...
		Для Каждого Стр Из МассивУслуг Цикл
			...
			МСтр = Документ.ДокументРеализации.СчетНаОплатуПокупателю.Состав.НайтиСтроки(Новый Структура("Номенклатура, Содержание", Стр.Номенклатура, Стр.Описание));
			Если МСтр.Количество() > 0 Тогда
				Стр.ЕдИзм = МСтр[0].ЕдиницаИзмерения;
				Стр.ТаблДок_ОКЕИ = Стр.ЕдИзм.Код; 
				Стр.ТаблДок_ЕдИзм = Строка(Стр.ЕдИзм);
				Стр.ПредСтрТабл_ЕдИзм = Строка(Стр.ЕдИзм);
				Стр.ПредСтрТабл_ОКЕИ = Стр.ЕдИзм.Код;
			КонецЕсли;
			...
			Если Стр.ЕдИзм = ЕдИзмУсл Тогда
				...
				Стр.ТаблДок_ОКЕИ = "";
				Стр.ТаблДок_ЕдИзм = "";
				Стр.ПредСтрТабл_ЕдИзм = "";
				Стр.ПредСтрТабл_ОКЕИ = "";
			КонецЕсли;
...
&НаСервере
Процедура С_СвернутьМассивУслуг(Ини, ИдентификаторМассива, Документ, ТипДокумента)
	
	МассивУслуг = Ини.мФайл.АктВР.мТаблДок[ИдентификаторМассива];
	
	Если МассивУслуг.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ЗаполнитьЕдИзмПоАктамАренды(МассивУслуг, Документ, ТипДокумента);
	
	СтруктЭталон = С_ПолучитьСтруктуруЭталонИзСтруктурыИсточника(МассивУслуг[0]);
	
	ТЗУсл = ПолучитьТЗЭталон(ТипДокумента);
	...
	Если ТипДокумента = 1 Тогда
		...
		ТЗАгр = ТЗУсл.Скопировать(,"Номенклатура, Описание, СуммаНДС, ТаблДок_Кол_во, ТаблДок_Сумма, СтавкаНДС");
		ТЗАгр.Свернуть("Номенклатура, Описание, СтавкаНДС", "СуммаНДС, ТаблДок_Кол_во, ТаблДок_Сумма");
		
	ИначеЕсли ТипДокумента = 2 Тогда 
		
		ТЗАгр = ТЗУсл.Скопировать(,"Номенклатура, Описание, СуммаНДС, СуммаНДСДо, ТаблДок_Кол_во, ПредСтрТабл_Кол_во, ТаблДок_Сумма, ПредСтрТабл_Сумма, СтавкаНДС, СтавкаНДСДо");
		ТЗАгр.Свернуть("Номенклатура, Описание, СтавкаНДС, СтавкаНДСДо", "СуммаНДС, ТаблДок_Кол_во, ТаблДок_Сумма, ПредСтрТабл_Кол_во, ПредСтрТабл_Сумма, СуммаНДСДо");

	КонецЕсли;

	МассивУслуг.Очистить();

	Если ТипДокумента = 1 Тогда
		
		Для Каждого СтрАгр Из ТЗАгр Цикл
			
			НСтрукт = С_ПолучитьСтруктуруЭталонИзСтруктурыИсточника(СтруктЭталон);
			
			МСтрТЗИсх = ТЗУсл.НайтиСтроки(Новый Структура("Номенклатура, Описание, СтавкаНДС", СтрАгр.Номенклатура, СтрАгр.Описание, СтрАгр.СтавкаНДС)); 
			СтрТЗИсх = МСтрТЗИсх[0];
			
			ЗаполнитьЗначенияСвойств(НСтрукт, СтрАгр);
			ЗаполнитьЗначенияСвойств(НСтрукт, СтрТЗИсх, "Цена, ЕдИзм, Название, СуммаАкциз, ТаблДок_ЕдИзм, ТаблДок_Идентификатор, ТаблДок_Код, ТаблДок_Название, ТаблДок_ОКЕИ, ТаблДок_Описание, ТаблДок_Тип, ТаблДок_Цена, мПараметр");
			
			С_ДопОбработкаСтруктурыУслуг(НСтрукт, ТипДокумента);
			
			МассивУслуг.Добавить(НСтрукт);
	...
	ИначеЕсли ТипДокумента = 2 Тогда 
		...
		Для Каждого СтрАгр Из ТЗАгр Цикл
			...
			МСтрТЗИсх = ТЗУсл.НайтиСтроки(Новый Структура("Номенклатура, Описание, СтавкаНДС, СтавкаНДСДо", СтрАгр.Номенклатура, СтрАгр.Описание, СтрАгр.СтавкаНДС, СтрАгр.СтавкаНДСДо)); 
			СтрТЗИсх = МСтрТЗИсх[0];
			
			ЗаполнитьЗначенияСвойств(НСтрукт, СтрАгр);
			ЗаполнитьЗначенияСвойств(НСтрукт, СтрТЗИсх, "Цена, ЕдИзм, Название, СуммаАкциз, ТаблДок_ЕдИзм, ТаблДок_Идентификатор, ТаблДок_Код, ТаблДок_Название, ТаблДок_ОКЕИ, ТаблДок_Описание, ТаблДок_Цена, ЦенаДо, СуммаАкцизДо, ПредСтрТабл_Цена, ПредСтрТабл_ОКЕИ, ПредСтрТабл_ЕдИзм, ПредСтрТабл");
...
&НаСервере
Процедура С_СвернутьТаблицыУслуг(Ини, Документ, ТипДокумента)
	
	Если ТипДокумента = 1 Тогда
		
		Если Ини.мФайл.АктВР.мТаблДок.АгентскиеУслуги.Количество() > 0 Тогда
			С_СвернутьМассивУслуг(Ини, "АгентскиеУслуги", Документ, ТипДокумента);
		КонецЕсли;
		
		Если Ини.мФайл.АктВР.мТаблДок.Услуги.Количество() > 0 Тогда
			С_СвернутьМассивУслуг(Ини, "Услуги", Документ, ТипДокумента);
		КонецЕсли;
...
&НаСервере
Процедура С_ДопОбработкаСтруктурыУслуг(НСтрукт, ТипДокумента)
	                   
	НСтрукт.СуммаНДС = Формат(НСтрукт.СуммаНДС, "ЧДЦ=2; ЧРД=.; ЧН=0.00; ЧГ=0");
	НСтрукт.ТаблДок_Кол_во = Формат(НСтрукт.ТаблДок_Кол_во, "ЧДЦ=3; ЧРД=.; ЧН=0.00; ЧГ=0");
	НСтрукт.ТаблДок_Сумма = Формат(НСтрукт.ТаблДок_Сумма, "ЧДЦ=2; ЧРД=.; ЧН=0.00; ЧГ=0");
	
	Если ТипДокумента = 2 Тогда
		НСтрукт.ПредСтрТабл_Кол_во = Формат(НСтрукт.ПредСтрТабл_Кол_во, "ЧДЦ=3; ЧРД=.; ЧН=0.00; ЧГ=0");
		НСтрукт.ПредСтрТабл_Сумма = Формат(НСтрукт.ПредСтрТабл_Сумма, "ЧДЦ=2; ЧРД=.; ЧН=0.00; ЧГ=0");
		НСтрукт.СуммаНДСДо = Формат(НСтрукт.СуммаНДСДо, "ЧДЦ=2; ЧРД=.; ЧН=0.00; ЧГ=0");
	КонецЕсли;
		
	Если Не ЗначениеЗаполнено(НСтрукт.ЕдИзм) Тогда
		НСтрукт.ТаблДок_Кол_во = "0.00";
		НСтрукт.Цена = Число(НСтрукт.ТаблДок_Сумма);
		
		Если ТипДокумента = 1 Тогда
			НСтрукт.мПараметр.Цена1С.Параметр_Значение = НСтрукт.ТаблДок_Сумма;
		КонецЕсли;
		
		Если ТипДокумента = 2 Тогда
			НСтрукт.ЦенаДо = Число(НСтрукт.ПредСтрТабл_Сумма);
			НСтрукт.ПредСтрТабл_Кол_во = "0.00";
		КонецЕсли;
	Иначе
		//
		//НСтрукт.Цена = Окр(Число(НСтрукт.ТаблДок_Сумма)/Число(НСтрукт.ТаблДок_Кол_во),2);
		//Если ТипДокумента = 1 Тогда
		//	НСтрукт.мПараметр.Цена1С.Параметр_Значение = Формат(НСтрукт.Цена, "ЧДЦ=2; ЧРД=.; ЧН=0.00; ЧГ=0");
		//КонецЕсли;
		//
		//Если ТипДокумента = 2 Тогда
		//	НСтрукт.ЦенаДо = Окр(Число(НСтрукт.ПредСтрТабл_Сумма)/Число(НСтрукт.ПредСтрТабл_Кол_во),2);
		//КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция С_ПолучитьСтруктуруЭталонИзСтруктурыИсточника(СтруктИст)
	
	СтруктЭталон = Новый Структура;
	
	Для Каждого Эл Из СтруктИст Цикл
		
		СтруктЭталон.Вставить(Эл.Ключ, Эл.Значение);
		
	КонецЦикла;
	
	Для Каждого Эл Из СтруктЭталон Цикл
		
		СтруктЭталон[Эл.Ключ] = Неопределено; 

	КонецЦикла;
	
	Возврат СтруктЭталон;
	
КонецФункции

&НаСервере
Функция ПолучитьТЗЭталон(ТипДокумента)
	...
	Если ТипДокумента = 1 Тогда
		...
		ТЗ.Колонки.Добавить("СуммаАкциз");
		ТЗ.Колонки.Добавить("СуммаНДС", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15, 2)));
		ТЗ.Колонки.Добавить("ТаблДок_ЕдИзм");
		ТЗ.Колонки.Добавить("ТаблДок_Идентификатор");
		ТЗ.Колонки.Добавить("ТаблДок_Код");
		ТЗ.Колонки.Добавить("ТаблДок_Кол_во", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15, 3)));
		ТЗ.Колонки.Добавить("ТаблДок_Название");
		ТЗ.Колонки.Добавить("ТаблДок_ОКЕИ");
		ТЗ.Колонки.Добавить("ТаблДок_Описание");
		ТЗ.Колонки.Добавить("ТаблДок_Сумма", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15, 2)));
		ТЗ.Колонки.Добавить("ТаблДок_Тип");
		ТЗ.Колонки.Добавить("ТаблДок_Цена");
		ТЗ.Колонки.Добавить("Цена");
		ТЗ.Колонки.Добавить("мПараметр");
	...
	ИначеЕсли ТипДокумента = 2 Тогда
		...
		ТЗ.Колонки.Добавить("ПредСтрТабл");
		ТЗ.Колонки.Добавить("ПредСтрТабл_ЕдИзм");
		ТЗ.Колонки.Добавить("ПредСтрТабл_Кол_во", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15, 3)));
		ТЗ.Колонки.Добавить("ПредСтрТабл_ОКЕИ");
		ТЗ.Колонки.Добавить("ПредСтрТабл_Сумма", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15, 2)));
		ТЗ.Колонки.Добавить("ПредСтрТабл_Цена");
		ТЗ.Колонки.Добавить("СтавкаНДСДо");
		ТЗ.Колонки.Добавить("СуммаАкцизДо");
		ТЗ.Колонки.Добавить("СуммаНДСДо", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15, 2)));
...
// Клес +
&НаСервере
Процедура ДополнитьШаблонДляАшан(ШаблонXML, СтруктураВложения)
	
	Если СтруктураВложения.Тип = "УпдСчфДоп" И СтруктураВложения.Документы1С.Количество() = 1 И ТипЗнч(СтруктураВложения.Документы1С[0].Значение) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
		
		КП = СтруктураВложения.Документы1С[0].Значение.ДокументОснование.АШАН_КодПодразделения;
		НЗ = СтруктураВложения.Документы1С[0].Значение.ДокументОснование.АШАН_НомерЗаказа;
		
		Если КП = "" ИЛИ НЗ = "" Тогда Возврат; КонецЕсли;
		
		Если НЗ = "" Тогда НЗ = "0"; КонецЕсли;
		
		СтрЗамены = "<ТекстИнф Идентиф=""подразделение"" Значен= """ + КП + """ />
        |	<ТекстИнф Идентиф=""номер_заказа_АРИБА"" Значен=""" + НЗ + """ />";
		
		Если СтрНайти(ШаблонXML, "</ИнфПолФХЖ1>") <> 0 Тогда 
			ШаблонXML = СтрЗаменить(ШаблонXML, "</ИнфПолФХЖ1>", СтрЗамены + "
      		|</ИнфПолФХЖ1>");
		Иначе
			ШаблонXML = СтрЗаменить(ШаблонXML, "</Документ>",
      		"</Документ>
     	   	|	<ИнфПолФХЖ1>" + "
     	   	|" + СтрЗамены + "
     	   	|</ИнфПолФХЖ1>");
