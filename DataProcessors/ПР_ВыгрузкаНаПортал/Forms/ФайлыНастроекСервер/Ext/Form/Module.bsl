...
Процедура	СбисВыбратьСоединение(Кэш,Отказ=Ложь) Экспорт
	...
	#Иначе
		Если Отказ Тогда
			...
			Возврат;
		КонецЕсли;
		ОткрытьФорму("Обработка.Д_ВыгрузкаНаПортал.Форма.ФайлыНастроекСервер",Новый Структура("ДанныеЗаполнить",ДанныеЗаполнить),Кэш.ГлавноеОкно,,,,Новый ОписаниеОповещения("СбисПрименитьУстановленныеНастройки",ЭтаФорма,ПараметрыВызова));
	#КонецЕсли
