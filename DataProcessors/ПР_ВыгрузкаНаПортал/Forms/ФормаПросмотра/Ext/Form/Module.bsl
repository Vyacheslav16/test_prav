...
Процедура ВывестиСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ВыборкаСФ, ЭтоУниверсальныйПередаточныйДокумент = Ложь)
	...
	Если ВыборкаСФ.Дата >= Дата("20170701000000") Тогда
...
Функция ПросмотрДок(Вид,МассивОбъект,ПараметрыПечати = Неопределено)
	...
	ИначеЕсли Вид = "УПД" Тогда
		ТабДок = ПечатьУПД(МассивОбъект,, ОбъектыПечати, ПараметрыПечати);
	КонецЕсли;	
	Возврат ТабДок;
...
Функция ПечатьУПД(МассивОбъектов,
	...
	ТекстЗапросаДокументам = Документы.СчетФактураВыданный.ТекстЗапросаПечатьСчетовФактур(Истина, Истина);
