...
Процедура АдаптироватьМКШапкаФормы()
	...
	Элементы.ГруппаСформироватьДекларациюОтказаться.Видимость = Ложь;
	
	ПредупреждениеМК = МобильныйКлиентБПФормаОбъекта.СоздатьПредупреждениеОбОграничениях(ЭтотОбъект,
		НСтр("ru = 'Отправить декларацию можно в версии для компьютера'"));
	Элементы.Переместить(ПредупреждениеМК, Элементы.ГруппаПодачаДекларации, Элементы.ФонТребуютсяРеквизиты);
...
Процедура УправлениеФормойМобильныйКлиент()
	...
	Элементы.АнализУчетаПоУСН.Видимость = Ложь;
