&НаСервере
Функция СформироватьПечатныйДокумент(ТзДокументыНаПечать = Неопределено,МассивТекДоков,ВернутьСтруктуру = Ложь, ПечатьБезЕдИзм = Ложь) Экспорт
	...
	Для Каждого ТекСтр Из ТзДокументыНаПечать Цикл
		...
		Для каждого СтрПечФормы Из ТекПечФормы Цикл
			...
			Пока КолвоКопий > 0 Цикл
				...
				ТабДок = ПросмотрДок(ПечФорма,МассивОбъект,ПараметрыПечати, ПечатьБезЕдИзм);
...
Функция ПросмотрДок(Вид,МассивОбъект,ПараметрыПечати = Неопределено, ПечатьБезЕдИзм = Ложь)
	...
	ИначеЕсли Вид = "Сч/ф" Тогда
		...
		ТабДок = ПечатьСчетовФактур1137(МассивОбъект, ОбъектыПечати, Документы.СчетФактураВыданный.ТекстЗапросаПечатьСчетовФактур(), ПараметрыПечати,ПечатьБезЕдИзм);
	КонецЕсли;	
	Возврат ТабДок;
...
Функция ПечатьСчетовФактур1137(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСчетамФактурам,
	...
	ДанныеСчетовФактур = ПолучитьДанныеДляПечатиСчетаФактуры1137(МассивОбъектов, ТекстЗапросаПоСчетамФактурам,,, ПараметрыПечати, ПечатьБезЕдИзм); //++ЭЛИАС 20120706
...
Функция ПолучитьДанныеДляПечатиСчетаФактуры1137(МассивОбъектов, ТекстЗапросаПоСчетамФактурам, ДополнитьДаннымиУПД = Ложь, ФормированиеЭД = Ложь,
	ПараметрыПечати = Неопределено, ПечатьБезЕдИзм = Ложь) Экспорт //++ЭЛИАС 20120706
...
Функция КолонкиГруппировкиТаблицыСчетаФактуры(ДляИдентификацииСтроки = Ложь, СворачиватьСтрокиИзРазныхОснований = Ложь)
	...
	Если ДляИдентификацииСтроки Тогда
...
Процедура ВывестиСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ВыборкаСФ, ЭтоУниверсальныйПередаточныйДокумент = Ложь) Экспорт
	...
	ОбластьМакета.Параметры.Заполнить(ДанныеШапки);
		...
		//ОбластьМакета.Параметры.ФИОРуководителя	= АР_ОбщиеПроцедуры.НастройкаПечатнойФормы(Шапка, Перечисления.ОтветственныеЛицаОрганизаций.Руководитель);
		//ОбластьМакета.Параметры.ФИОГлавногоБухгалтера = АР_ОбщиеПроцедуры.НастройкаПечатнойФормы(Шапка, Перечисления.ОтветственныеЛицаОрганизаций.ГлавныйБухгалтер);
		...
		Если ЗначениеЗаполнено(ДвоичныеДанныеПечать)
			...
			Если ДвоичныеДанныеПодписьРуководителя.Получить() <> Неопределено Тогда
				...
				ОбластьМакета.Рисунки.ФаксимилеРуководитель.ЦветФона = Новый Цвет();
				ОбластьМакета.Рисунки.ФаксимилеРуководитель.Линия    = Новый Линия(ТипЛинииРисункаТабличногоДокумента.НетЛинии);
			КонецЕсли;
			//бух
