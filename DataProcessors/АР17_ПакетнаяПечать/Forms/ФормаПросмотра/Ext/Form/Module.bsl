...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
   	ИтогоТабДок = Параметры.ИтогоТабДок;
	
	ТЗДок = Новый ТаблицаЗначений;
	ТЗДок = Параметры.ТабДокументов.Выгрузить();
	Если ТЗДок.Колонки.Найти("Пометка") <> Неопределено Тогда
		ТЗДок.Колонки.Добавить("Печатать",Новый ОписаниеТипов("Булево"));
		ТЗДок.ЗагрузитьКолонку(ТЗДок.ВыгрузитьКолонку("Пометка"),"Печатать");
	КонецЕсли;	
	ТабДокументов.Загрузить(ТЗДок);
	
	ПечФормы = Новый ТаблицаЗначений;
	ПечФормы = Параметры.ПечатныеФормы.Выгрузить();
	ПечатныеФормы.Загрузить(ПечФормы);
		
	ВидДокумента = Параметры.ВидДокумента;
	Объект.ПечатьБезЕдИзм = Параметры.ПечатьБезЕдИзм;
КонецПроцедуры
		...
		Для Каждого Стр Из ТабДокументов Цикл
			...
			Для Каждого ПечФормы Из ПечатныеФормы Цикл
				...
				ИначеЕсли ПечФормы.ПечатнаяФорма = "Сч/ф" и ПечФормы.Печать = Истина Тогда
					...
					Пока Количество > 0 Цикл
						Если ТипЗнч(Стр.Документ) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда
							СФ = Новый Массив;
							СФ.Добавить(Стр.Документ);
						Иначе	
...
Функция НапечататьРеестр(ТабДок)
	...
	Для Каждого Стр из ТабДокументов Цикл
		...
		Если ТипЗнч(Стр.Документ) = Тип("ДокументСсылка.АР17_СчетНаАренду") Тогда
			Область.Параметры.Ссылка = АР_РаботаСоШтрихКодами.ПолучитьРегистратор(Стр.Документ);
		...
		ИначеЕсли ТипЗнч(Стр.Документ) = Тип("ДокументСсылка.АР17_НачислениеАренднойПлаты") Тогда
			...
			Если НайтиПодчиненнуюРеализацию(Стр.Документ) <> Неопределено Тогда
				...
				Область.Параметры.Ссылка = "Акт №" + Реализация.Номер + " от " + Формат(Реализация.Дата,"ДЛФ=Д");
			Иначе
...
Функция ПечатьСчетовФактур1137(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСчетамФактурам,
	...
	ДанныеСчетовФактур = Обработки.АР17_ПакетнаяПечать.ПолучитьДанныеДляПечатиСчетаФактуры1137(МассивОбъектов,  Документы.СчетФактураВыданный.ТекстЗапросаПечатьСчетовФактур(),, ПараметрыПечати, Объект.ПечатьБезЕдИзм); //++ЭЛИАС 20120706
...
Функция ПолучитьДанныеДляПечатиСчетаФактуры1137(МассивОбъектов, ТекстЗапросаПоСчетамФактурам, ДополнитьДаннымиУПД = Ложь,
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		Если ТипЗнч(ПараметрыПечати) = Тип("Структура") И ПараметрыПечати.Свойство("Подписанты") Тогда
			...
			Руководитель	= НастройкаПечатнойФормы(ВыборкаСФ, Перечисления.ОтветственныеЛицаОрганизаций.Руководитель);
			Бухгалтер		= НастройкаПечатнойФормы(ВыборкаСФ, Перечисления.ОтветственныеЛицаОрганизаций.ГлавныйБухгалтер);
...
Функция ПросмотрДок(Вид,МассивОбъект,ПараметрыПечати = Неопределено)
	ОбъектыПечати = Новый СписокЗначений;
	Если Вид = "Счет" Тогда
		ТабДок = Документы.АР17_СчетНаАренду.ПечатьСчета(МассивОбъект,ОбъектыПечати,Истина,Объект.ПечатьБезЕдИзм);
