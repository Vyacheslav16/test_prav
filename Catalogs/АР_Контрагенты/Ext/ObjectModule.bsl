...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ГоловнойКонтрагент) И НЕ ОбособленноеПодразделение Тогда
		...
		Если ЭтоНовый() Тогда
			НоваяСсылка = Справочники.АР_Контрагенты.ПолучитьСсылку();
...
Процедура ПриЗаписи(Отказ)
	...
	Если ЭтотОбъект.ДополнительныеСвойства.Новый Тогда
		ВыборкаКонтрагент = ИННКППУжеИспользуетсяВИнформационнойБазе(ЭтотОбъект.ИНН,ЭтотОбъект.КПП,"Контрагенты",ЭтотОбъект.Ссылка);
		Если ВыборкаКонтрагент = Неопределено Тогда
			Спр = Справочники.Контрагенты.СоздатьЭлемент();
			//Спр.Заполнить(ЭтотОбъект.Ссылка);
			ЗаполнитьЗначенияСвойств(Спр,ЭтотОбъект,,"Владелец,Код");
			//Спр.Записать();
		КонецЕсли;	
	КонецЕсли;
	//-са
