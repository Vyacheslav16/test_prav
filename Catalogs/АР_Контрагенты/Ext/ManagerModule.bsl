...
Функция ТекстЗапросаПроверкиДублейКонтрагентов()
	...
	ТекстЗапроса = 
	"ВЫБРАТЬ
	|	АР_Контрагенты.Ссылка
	|ИЗ
	|	Справочник.АР_Контрагенты КАК АР_Контрагенты
	|ГДЕ
	|	НЕ АР_Контрагенты.ЭтоГруппа
	|	И НЕ АР_Контрагенты.Ссылка = &Ссылка
	|	И АР_Контрагенты.СтранаРегистрации = ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия)
	|	И АР_Контрагенты.КПП = &КПП
	|	И АР_Контрагенты.ИНН = &ИНН";
...
Функция ЕстьЗаписиВРегистреДублей(ИНН, КПП, Ссылка) Экспорт
	...
	|	Контрагенты.Ссылка
	|ИЗ
	|	Справочник.АР_Контрагенты КАК Контрагенты
	|ГДЕ
	|	Контрагенты.ИНН = &ИНН
...
Функция ПрочитатьДанныеXMLКарточки(АдресXMLФайла)
	...
	Если НЕ Карточка.Тип() = ФорматыЭДО.ПолучитьТипЗначенияCML("Контрагент") Тогда
		Возврат Неопределено;
	КонецЕсли;
